...
if [ "${USE_CHANGED_FILES}" = "true" ]; then
    echo "${CHANGED_FILES}" | xargs -rt ${INPUT_PHPCS_BIN_PATH} --standard=PSR2 ${ENABLE_WARNINGS_FLAG} --report=checkstyle
else
    ${INPUT_PHPCS_BIN_PATH} --standard=PSR2 ${ENABLE_WARNINGS_FLAG} --report=checkstyle
fi
